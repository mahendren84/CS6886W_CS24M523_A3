...
def parse_args():
    ...
    parser.add_argument(
        "--weight-per-channel",
        action="store_true",
        help="Use per-output-channel weight scales (better accuracy at low bits).",
    )
    parser.add_argument(
        "--no-pack-weights",
        action="store_true",
        help="Disable 2/4-bit packing (keeps weights as int8 buffers).",
    )
    parser.add_argument(
...
def main():
    ...
    cfg = ckpt.get("config", {})
    # Data (match the normalization used during training when available)
    chosen_norm = cfg.get("norm", "cifar")
    ...
    if args.weight_quant_bits > 0:
        quant_model = quantize_model_weights(
            ...
            per_channel=args.weight_per_channel,
            pack=(not args.no_pack_weights),
        )
